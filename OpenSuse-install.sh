...
$packer alacritty bash-completion bat blueman brightnessctl btop cpuid cups curl dconf-editorduf dunst fastfetch feh file-roller firefox fish flameshot font-manager fzf galculator gcc gcolor2 gimp git gnome-disk-utility gparted gsettings-desktop-schemas gzip harfbuzz-devel htop i3lock kitty libreoffice libreoffice-l10n-pl libX11-devel libXft-devel libXinerama-devel libxcb-res0 lightdm lightdm-gtk-greeter-settings lsd mako meld mlocate MozillaThunderbird neovim NetworkManager-applet ncurses-devel numlockx nwg-look opi os-prober p7zip papirus-icon-theme parcellite pavucontrol pdfarranger picom polkit polkit-gnome ranger rclone ripgrep rofi rsync scrot sensors starship sxhkd sxiv system-config-printer tealdeer tealdeer-fish-completion thunar thunar-volman time tree ueberzugpp unrar unzip vim vlc wget xclip xdg-user-dirs xfce4-notifyd xinit xorg-x11-driver-video xorg-x11-essentials xorg-x11-fonts xorg-x11-fonts-converted xorg-x11-fonts-core xorg-x11-fonts-legacy xorg-x11-libX11-ccache xorg-x11-server xorg-x11-server-extra xorg-x11-server-Xvfb xorg-x11-Xvnc xorg-x11-Xvnc-module xorgproto-devel xwininfo yazi zathura zathura-plugin-pdf-poppler zoxide
