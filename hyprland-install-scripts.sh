...
COMMON_PACKAGES=(
    azote bash-completion bat blueman btop brightnessctl cliphist cronie cups curl file-roller firefox fish floorp-bin foot fzf galculator gcc gcolor3 gnome-disk-utility gparted grimblast-git grub-customizer gzip htop hyprpicker i3lock iptables kitty mako mc meld neovim numlockx nwg-look os-prober pamixer parcellite pavucontrol pdfarranger picom polkit-gnome qemu-full qemu-img qt5ct ranger rclone ripgrep rofi rofi-emoji rsync sane sddm scrot simple-scan slurp starship stow sxhkd swappy swaybg swaylock-effects-git swaylock-fancy-git system-config-printer tela-circle-icon-theme-all thunar thunar-archive-plugin thunar-volman time timeshift trash-cli tree tumbler unrar unzip virt-install virt-manager virt-viewer vlc waybar wget wl-clipboard wlogout wofi xfce4-notifyd xfce4-terminal xclip xdg-user-dirs xreader yazi yt-dlp zathura zathura-pdf-mupdf zoxide
)
