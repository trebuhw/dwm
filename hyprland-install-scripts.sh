...
COMMON_PACKAGES=(
    azote bash-completion bat blueman btop brightnessctl cliphist cronie cups curl file-roller firefox fish foot fzf galculator gcc gcolor3 gnome-disk-utility gparted grub-customizer gzip htop hyprpicker hyprland i3lock iptables kitty mako mc meld neovim numlockx nwg-look os-prober pamixer parcellite pavucontrol pdfarranger picom polkit-gnome qemu-full qemu-img qt5ct ranger rclone ripgrep rofi rofi-emoji rsync sane sddm scrot simple-scan slurp starship stow sxhkd swappy swaybg system-config-printer tela-circle-icon-theme-all thunar thunar-archive-plugin thunar-volman time timeshift trash-cli tree tumbler unrar unzip virt-install virt-manager virt-viewer vlc waybar wget wl-clipboard wofi xfce4-notifyd xfce4-terminal xclip xdg-user-dirs xreader yazi yt-dlp zathura zathura-pdf-mupdf zoxide
)
...
case "$DISTRO" in
    arch)
        ...
        YAY_PACKAGES=(
        bibata-cursor-theme brother-dcp1610w brscan4 google-chrome grimblast-git moc-pulse mkinitcpio-firmware simple-sddm-theme-git sublime-text-4 waypaper wlogout)
        ;;
    ubuntu)
